class openstack::y004_neutron (
  ...
  $admin_network_fqdn       = hiera('admin_network_fqdn'),
  $public_network_fqdn      = hiera('public_network_fqdn'),
  ...
  class { '::neutron::agents::ml2::ovs':
    tunnel_types               => ['vxlan'],
    vxlan_udp_port             => '4789',
    l2_population              => $router_distributed,
    arp_responder              => $router_distributed,
    enable_distributed_routing => $router_distributed,
  ...
  if $router_distributed == true or $::hostname =~ /^*controller-\d*$/ {
